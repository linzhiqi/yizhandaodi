...
public class QuestionManager {
	...
	private Boolean isRandom = true; // false means pick questions with the order
									// of the field "_id"
	...
	public boolean checkAnswer(String inputA){
		...
		switch(q.getCheckId()){
		case 0:
			return ContainsCaseInsensitive(getAnswerList(q.getAnswer()),inputA);
		case 1:
			return ContainsSameCharSetCaseInsensitive(getAnswerList(q.getAnswer()),inputA);
		default:
			return ContainsCaseInsensitive(getAnswerList(q.getAnswer()),inputA);
		}
	...
	private Question getNextQObject(Context ct, boolean isDiffChange) {
		...
		if ((this.qCache==null)||(!this.qIt.hasNext())||(isDiffChange)) {
			if (refreshQCache(ct) > 0) {
				getNextQObject(ct, isDiffChange);
			} else {
		...
		if(this.isRandom){
			...
			DBHelper myDbHelper = new DBHelper(ct);
			try {
	...
	private int refreshQCache(Context ct) {
		...
		ArrayList<Question> questions =(ArrayList<Question>)myDbHelper.getQuestionSet(this.difficulty, this.category, this.topic, this.isRandom, this.startQId, Constants.CACHESIZE);
		myDbHelper.close();
